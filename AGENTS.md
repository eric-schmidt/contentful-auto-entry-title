...
The `-ci` variants pass `--token $CONTENTFUL_ACCESS_TOKEN` explicitly, so _they_ would work under any name; `scripts/import-content-model.mts` reads it directly too. The interactive commands are the binding constraint. Keep all four on the one name.
